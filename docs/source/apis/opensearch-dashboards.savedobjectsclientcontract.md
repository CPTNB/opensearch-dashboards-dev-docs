...
[Home](./index.md) &gt; [opensearch-dashboards](./opensearch-dashboards.md) &gt; [SavedObjectsClientContract](./opensearch-dashboards.savedobjectsclientcontract.md)

## SavedObjectsClientContract type

Saved Objects is OpenSearchDashboards's data persisentence mechanism allowing plugins to use OpenSearch for storing plugin state.
...
1. Caused by bad implementation (ie. undefined is not a function) and as such unpredictable 2. An error that has been classified and decorated appropriately by the decorators in [SavedObjectsErrorHelpers](./opensearch-dashboards.savedobjectserrorhelpers.md)
...
Type 2 errors are decorated versions of the source error, so if the opensearch client threw an error it will be decorated based on its type. That means that rather than looking for `error.body.error.type` or doing substring checks on `error.body.error.reason`<!-- -->, just use the helpers to understand the meaning of the error:
...
From the perspective of application code and APIs the SavedObjectsClient is a black box that persists objects. One of the internal details that users have no control over is that we use an opensearch index for persistance and that index might be missing.
...
See [SavedObjectsClient](./opensearch-dashboards.savedobjectsclient.md) See [SavedObjectsErrorHelpers](./opensearch-dashboards.savedobjectserrorhelpers.md)
...
```
<b>References:</b> [SavedObjectsClient](./opensearch-dashboards.savedobjectsclient.md)
